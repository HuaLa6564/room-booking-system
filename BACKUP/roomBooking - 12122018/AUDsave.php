...
<?php
	if(isset($_POST['name'])){
		if(empty($_POST['name'])|| empty($_POST['id'])||empty($_POST['audience']) ||empty($_POST['date']) ||empty($_POST['tstart']) ||empty($_POST['tend']))
			echo "<script>alert('Please Do It Again!'); window.location='AUDform.php'</script>";
		else {
		   $roomNo = $_POST['roomNo'];
		   $name = $_POST['name'];
		   $id = $_POST['id'];
		   ...
		   $code = $_POST['codela'];
			...
			echo "<script>alert('Thank you your data has been saved !'); window.location='AUDhome.php'</script>";
